...
home_branch VARCHAR(50), name VARCHAR(50),
email VARCHAR(50),
DOB DATE,
house_address VARCHAR(50), city VARCHAR(50),
state VARCHAR(50),
balance
INT,acc_no
INT,
...
insert into Customer (customer_id, home_branch, name, email, DOB, house_address, city, state,balance,acc_no) values (237, 'Piraquara', 'Ingram Keeting', 'ikeeting3@printfriendly.com',
'27- FEBRUARY-1987', '39', 'Vidon Way', 'California', 3236662,9827);
insert into Customer (customer_id, home_branch, name, email, DOB, house_address, city, state,balance,acc_no) values (478, 'Áyios Yeóryios', 'Emmie Gartrell', 'egartrell4@rediff.com',
'04- MARCH-1991', '47', 'Washington Alley', 'California', 1069704,3231);
...
insert into Customer (customer_id, home_branch, name, email, DOB, house_address, city, state,balance,acc_no) values (123, 'Beijiang', 'Jay Ryles', 'jryles6@blog.com', '27-
MARCH-1993', '71378','Express Lane', 'California', 3726391,1300);
...
insert into Customer (customer_id, home_branch, name, email, DOB, house_address, city, state,
balance,acc_no) values (53, 'Nanxindian', 'Pansie Stoneley', 'pstoneleya@constantcontact.com', '18-MAY-1989', '60209', 'Sullivan Avenue', 'California', 4151444,7663);
insert into Customer (customer_id, home_branch, name, email, DOB, house_address, city, state, balance,acc_no) values (759, 'Pinamalayan', 'Donni McSweeney', 'dmcsweeneyb@sciencedirect.com','28-APRIL-1988', '92', 'Paget Point', 'California', 3032477,9829); insert into Customer (customer_id, home_branch, name, email, DOB, house_address, city, state,balance,acc_no) values (427, 'Yevlakh', 'Micah Tomkins', 'mtomkinsc@taobao.com', '31- DECEMBER-1988', '6', 'Crownhardt Drive', 'California', 2885458,3232);
...
insert into Customer (customer_id, home_branch, name, email, DOB, house_address, city, state, balance,acc_no) values (52, 'Barrie', 'Winonah Ship', 'wshipt@google.co.uk', '13-FEBRUARY-1992',

  '26499', 'Norway Maple Drive', 'California', 497033,2009);
--Complaint Table 
create table
Complaint( Status
VARCHAR(50),
...
type VARCHAR(100),
no_of_days INT,
foreign key(customer_id) references Customer(customer_id),primary key(customer_id,type)
);
insert into Complaint (Status,customer_id,type,no_of_days) values ('false',785,'debit card',3); insert into Complaint (Status,customer_id,type,no_of_days) values ('true',52,'transaction issues',7);insert into Complaint (Status,customer_id,type,no_of_days) values ('false',108,'credit card',14); insert into Complaint (Status,customer_id,type,no_of_days) values ('false',266,'debit card',16);
insert into Complaint (Status,customer_id,type,no_of_days) values ('false',946,'behaviour issues withthe staff',1);
insert into Complaint (Status,customer_id,type,no_of_days) values ('true',60,'transaction issues',12);insert into Complaint (Status,customer_id,type,no_of_days) values ('true',250,'debit card',20);
insert into Complaint (Status,customer_id,type,no_of_days) values ('false',427,'credit card',2); insert into Complaint (Status,customer_id,type,no_of_days) values ('true',968,'behaviour issues withthe staff',5);
insert into Complaint (Status,customer_id,type,no_of_days) values ('true',712,'debit card',11); insert into Complaint (Status,customer_id,type,no_of_days) values ('true',96,'transaction issues',13);insert into Complaint (Status,customer_id,type,no_of_days) values ('true',844,'credit card',5);
...
Loan_amount INT,
customer_id INT,
foreign key(customer_id) references Customer(customer_id),primary key(customer_id,Loan_number)
);
...
phone_no INT,
customer_id INT,
foreign key(customer_id) references Customer(customer_id),primary key(customer_id)
);
--CUSTOMER CANNOT OPEN A BANK ACCOUNT ON SUNDAY CREATE OR REPLACE TRIGGER Customer_SUNDAY
BEFORE INSERT OR UPDATE OR DELETE ON CUSTOMER BEGIN
IF RTRIM(UPPER(TO_CHAR(SYSDATE,'DAY')))='SUNDAY' THEN RAISE_APPLICATION_ERROR(-20022,'NO OPERARTION CAN BE PERFORMED ON
SUNDAY') ;END IF;
END;
--LOAN CANNOT BE GIVEN ON SUNDAY
CREATE OR REPLACE TRIGGER LOAN_SUNDAY BEFORE INSERT OR UPDATE OR DELETE ON LOAN BEGIN
IF RTRIM(UPPER(TO_CHAR(SYSDATE,'DAY')))='SUNDAY' THEN RAISE_APPLICATION_ERROR(-20022,'NO OPERARTION CAN BE PERFORMED ON
SUNDAY') ;END IF;
END;
--COMPLAINT CANNOT BE LODGED ON SUNDAY CREATE OR REPLACE TRIGGER Complaint_SUNDAY BEFORE INSERT OR UPDATE OR DELETE ON Complaint BEGIN
IF RTRIM(UPPER(TO_CHAR(SYSDATE,'DAY')))='SUNDAY' THEN RAISE_APPLICATION_ERROR(-20022,'NO OPERARTION CAN BE PERFORMED ON
SUNDAY') ;END IF;
END;
--IF NEW BALANCE IS LESS THAN 0, IT WILL RAISE A WARNING CREATE OR REPLACE TRIGGER UPDATE_balance
BEFORE UPDATE ON Customer
FOR EACH ROW
BEGIN
IF :NEW.balance<0 THEN

  RAISE_APPLICATION_ERROR(-20000,'NEW BALANCE CANNOT BE LESS THAN 0'); END IF;
END;
--IF LOAN AMOUNT BECOMES 0, LOAN RECORD IS DELETEDCREATE OR REPLACE TRIGGER LOAN_AMT AFTER UPDATE ON LOAN
FOR EACH ROW
BEGIN
IF :NEW.loan_amount= 0 THEN
DELETE FROM LOAN WHERE Loan_amount = 0;END IF;
END;
--REDIRECTING COMPLAINT TO MANAGER IF 15 DAYS HAS BEEN PASSED DECLARE
...
--TO SEE PENDING COMPLAINTS OF CUSTOMERS WITH NO OF DAYS>15 SELECT * FROM MANAGER;
--TOTAL LOAN GIVEN BY BANK
CREATE OR REPLACE FUNCTION TOTAL_LOAN RETURN NUMBER IS s NUMBER;
BEGIN
Select sum(LOAN_AMOUNT) into s from
LOAN;RETURN(s);
END;
DECLARE
C
NUMBER
BEGIN
C:=TOTAL_LOAN(); DBMS_OUTPUT.PUT_LINE('TOTAL LOAN '||C); END;
--CALCULATES TOTAL LOAN TAKEN BY A CUSTOMER DECLARE
TOTAL NUMBER;
PROCEDURE TOTAL_LOAN_CUST( CUSTID IN NUMBER) IS BEGIN
SELECT SUM(*) INTO TOTAL FROM LOAN

GROUP BY LOAN.CUSTOMER_ID
WHERE
CUSTID=LOAN.CUSTOMER_ID;BEGIN TOTAL_LOAN_CUST(131);
DBMS_OUTPUT.PUT_LINE('TOTAL LOAN AMOUNT IS '||TOTAL); END;
--CHECKING WHETHER A PARTICULAR CUSTOMER HAS TAKEN LOAN OR NOT DECLARE
CHECK_LOAN
EXCEPTIONBEGIN
IF NOT EXISTS ( SELECT * FROM LOAN WHERE CUSTOMER_ID = 92) THENRAISE CHECK_LOAN
END IF;
EXCEPTION
WHEN CHECK_LOAN THEN DBMS_OUTPUT.PUT_LINE('NO LOAN RECORD FOUND');
END;